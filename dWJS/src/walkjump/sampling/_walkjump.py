...
def stack_seed_sequences(seeds: list[str], num_samples: int) -> torch.Tensor:
    """
    Convert a list of seed sequences to a collection of initial points for sampling trajectory.
...
def walk(
    ...
    for seed_chunk in seed_tensor.chunk(chunksize):
        ...
        original_seed_chunk = seed_chunk
...
def walkjump(
    ...
    if mask_idxs is not None and len(mask_idxs) > 0: #if mask_idxs:
