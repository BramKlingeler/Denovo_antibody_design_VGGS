...
def instantiate_redesign_mask(redesign_regions: Iterable[str]) -> list[int]:
    ...
    mask_idxs = sorted(
        set(range(0, LENGTH_FV_HEAVY_AHO )) - redesign_accumulator #+ LENGTH_FV_LIGHT_AHO
    )
    return mask_idxs
'''
...
def instantiate_redesign_mask(redesign_regions: Iterable[str]) -> list[int]:
    ...
    # if redesign_regions is set to null, no mask
    if not redesign_regions:
        return []
    ...
    for region in redesign_regions:
        ...
        redesign_accumulator |= set(range(aho_start, aho_end))
        # assert chain == "H", f"Light-chain region {region} in heavy-only model"

        # aho_start, aho_end = RANGES_AHO[region]
        # redesign_accumulator |= set(range(aho_start, aho_end))

    mask_idxs = sorted(
        set(range(0, LENGTH_FV_HEAVY_AHO)) - redesign_accumulator
    )
    return mask_idxs
