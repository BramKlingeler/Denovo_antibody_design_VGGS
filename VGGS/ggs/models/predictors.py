...
class LengthMaxPool1D(nn.Module):
    def __init__(self, in_dim, out_dim, linear=False, activation='relu'):
        ...
        if activation == 'swish':
            ...
            self.act_fn = nn.ReLU() #lambda x: F.relu(x)
        elif activation == 'elu':
            self.act_fn = nn.ELU() #lambda x: F.elu(x)        
        else:
...
class BaseCNN(nn.Module):
    def __init__(
            self,
            n_tokens: int = 21, #for gap 21, default 20
            kernel_size: int = 5,
            input_size: int = 256, #default 256
            dropout: float = 0.0,
            make_one_hot=True,
            activation: str = 'relu',
            linear: bool=True,
        ...
        '''
        Increase model with an extra layer, has been taken out
        nn.Sequential(
            nn.Conv1d(n_tokens, input_size, kernel_size=kernel_size, padding=kernel_size//2),
            nn.ReLU(),
            nn.Conv1d(input_size, input_size, kernel_size=kernel_size, padding=kernel_size//2),
            nn.ReLU(),
