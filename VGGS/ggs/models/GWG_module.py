...
#from ggs.data.utils.ab_tokenize import GGS_AntibodyTokenizer
...
class GwgPairSampler(torch.nn.Module):
    ...
    def __init__(
        ...
        self.predictor_tokenizer =Encoder() #for gap token
    ...
    def _gibbs_sampler(self, seq_one_hot):
        ...
        def _gwg_sample():
            ...
            # --- Stable OneHotCategorical fix ---
            logits = delta_ij.flatten() / self.temp

            # numerical stabilization: subtract max
            logits = logits - logits.max(dim=-1, keepdim=True)[0]

            # convert logits safely into probabilities
            probs = torch.clamp(torch.softmax(logits, dim=-1), min=1e-8)
            probs = probs / probs.sum(dim=-1, keepdim=True)

            # now build the categorical distribution
            gwg_proposal = dists.OneHotCategorical(logits = probs)
            ...
            # [num_samples, L, 21] # 21 for gap tokens
    ...
    def _metropolis_hastings(
        ...
        # [num_seq, L, 21] # for gap tokens [num_seq, L, 21]
