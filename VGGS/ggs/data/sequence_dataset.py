...
class PreScoredSequenceDataset(Dataset):
    ...
    def cluster(self):
        """
        Scalable replacement for hierarchical clustering.
        Uses greedy farthest-point sampling with Hamming distance.
        """

        sequences = self.sequences
        scores = np.array(self.scores)
        n_clusters = self._cluster_cutoff

        # Encode sequences once (fast Hamming)
        alphabet = "ARNDCQEGHILKMFPSTWYV-"
        enc = np.array([[alphabet.index(c) for c in s] for s in sequences])

        # Start from highest-score sequence
        centers = [np.argmax(scores)]
        min_dists = np.full(len(sequences), np.inf)

        def hamming(a, b):
            return np.count_nonzero(a != b)

        # Greedy farthest-point selection
        for _ in range(1, min(n_clusters, len(sequences))):
            last_center = enc[centers[-1]]

            # Update min distance to any center
            dists = np.count_nonzero(enc != last_center, axis=1)
        ...
        # Keep only selected representatives
        self._data = self._data.iloc[centers].reset_index(drop=True)

        self._log.info(
            f"Clustered to {len(self.sequences)} sequences "
            f"with scores min={np.min(self.scores):.2f}, "
            f"max={np.max(self.scores):.2f}, "
            #f"mean={np.mean(self.scores):.2f}, "
            #f"std={np.std(self.scores):.2f}"
        )
