...
#from ggs.data.utils.ab_tokenize import GGS_AntibodyTokenizer #replacement for gaptokens
import glob
...
alphabet = "ARNDCQEGHILKMFPSTWYV-" #Gap token "-"
...
def diversity(seqs):
    ...
    for i in range(num_seqs):
        for j in range(num_seqs):
            ...
            #real_total_dist += levenshtein(x.replace("-", ""), y.replace("-", ""))
        avg_dist = total_dist / (num_seqs*(num_seqs-1))
        #real_avg_dist = real_total_dist / (num_seqs*(num_seqs-1))
    return total_dist / (num_seqs*(num_seqs-1)) #avg_dist, real_avg
...
class EvalRunner:
    def __init__(self, runner_cfg):
        ...
        self.predictor_tokenizer = Encoder() #GGS_AntibodyTokenizer()
        ...
        if self._cfg.predictor_dir is not None:
            predictor_path = os.path.join(self._cfg.predictor_dir, 'predictor.ckpt') #fill in the desired predictor
