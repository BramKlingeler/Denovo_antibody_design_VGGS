...
- Experiments: COV-oracle, COV-easy-smoothed, COV-easy-unsmoothed, 
...
Running ```ggs/train_predictor.py``` uses the config file ```configs/train_predictor.yaml```. This file can be used to modify various aspects of the predictor (architecture of the CNN, batch size, validation set size, etc.). Setting ```experiment=train/COV-easy-unsmoothed``` uses the config file ```configs/experiment/train/COV-easy-smoothed```. This file contains the settings for the GFP hard task from the paper. Analogous files can be made to test out other task difficulties (e.g. changing the percentile cutoffs or mutation gap). The general structure here is maintained for the other experiment types. 
...
- Experiments: COV-easy. Naming may be slightly different at this time
...
- Experiments: COV-easy-smoothed, COV-easy-unsmoothed.
